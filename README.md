...
面向 Apple 中国大陆官网、京东、天猫和淘宝的本机购买辅助工程。使用正式版 Chrome，保留登录会话，提供商品检查、规格优先级、结算核对、24 期零费用分期核验、单次提交和人工接管。**当前未完成任何平台在程序独立 profile 中的真实自动下单验收。** 本轮 Apple 使用真实适配器完成离线合成页面的受保护演练；新品 Continue 分支仍暂停。普通 Chrome 的历史待付款单不代表程序可用，详细边界见 [验收记录](VALIDATION.md)。
...
## 当前状态与证据（2026-09-11 审核修复）

| 项目 | 本轮结果 | 证据边界 |
|---|---|---|
| 全量 pytest / Ruff / compileall | PASS | 239 passed，3 warnings；正式 Chrome 本地夹具、模拟故障和单元测试，未提交真实订单 |
...
| Apple 受保护演练 | PASS（合成页面） | 真实适配器、临时程序 profile 到 READY_TO_SUBMIT；提交与付款点击 0 |
...
| 本机状态页 | PASS（API/HTTP） | 新版 8766 返回演练保护及确认按钮；本轮未用浏览器点击 UI |
| Apple 新品 Continue / 程序实站完整交易 | NOT RUN | Chrome 工具连接不可用；保留暂停，不猜流程，不清既有订单锁 |
...
结算暂停时先在官网核对当前收货地址与国行版本，再输入 `confirm-address apple`、`confirm-market apple`（或网页上的同名确认按钮），最后 `resume apple`。确认只绑定本轮可见地址与商品的本地摘要；页面内容变化后必须重新确认。不会把配送地区、店铺名称或中国商店网址自动当作国行版本证明。
...
GET：`/status`、`/platforms`、`/products`、`/events?limit=100`、`/orders?limit=100`、`/health`。POST：`/start`、`/stop`、`/resume`、`/login`、`/check-login`、`/save-target`、`/check-product`、`/check-checkout`、`/read-order`、`/confirm-address`、`/confirm-market`。两个确认接口必须显式传 `confirmed:true`，不能启用提交。`/start` 可传 `dry_run:true` 强制演练。接口说明在 `/docs`；拒绝密码或 Token 等额外字段。订单列表只返回订单号后四位，完整引用和提交前摘要留在本机 SQLite。
