...
STAGES = (("prepare", -600), ("open_product", -180), ("ready", -30),
          ("warmup", -5), ("sale", 0))


def backoff(
    attempt: int, base: float, maximum: float, jitter: float = 0.1, retry_after: float = 0,
) -> float:
...
class Scheduler:
    ...
    def __init__(self, sale_time: datetime | None, stop_event: asyncio.Event):
